            ...
            if(flag)
            {
                ...
                else
                {
                    printf("%d bytes from %s (h: %s) (%s) msg_seq=%d ttl=%d rtt = %Lf ms.\n", 
                          ping_packet_size, ping_dom, rev_host, ping_ip, msg_count, ttl_val, rtt_msec);
...
void *exe_ping(void* argv_void){
    ...
    ip_addr = dns_lookup(argv, &addr_con);
    if(ip_addr == NULL)
    {
    ...
    send_ping(sockfd, &addr_con, reverse_hostname, ip_addr, argv);
    pthread_exit(0);
}
...
int main(int argc, char *argv[]){
    ...
    puts("Enter ping packet size");
    scanf("%d", &ping_packet_size);
    
    int i = 1, thread_count = 0;
    while (argv[i])
    {
        thread_count ++;
        i++;
    ...
    for(int i = 0 ; i < thread_count ; i++){
        pthread_create(&tid[i], NULL, exe_ping, (void *)&argv[i+1]);
    }
